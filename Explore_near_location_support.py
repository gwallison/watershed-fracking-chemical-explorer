...
print('** IN gwa_local PYTHON SUPPORT **')
...
def process_lat_lon_input(lat_lon_str):
    try:
        lst = lat_lon_str.split(',')
        assert len(lst)==2
        lat = float(lst[0])
...
def get_disclosures(df,lat,lon,radius_in_feet=5280):
    radius_m = radius_in_feet * 0.3048
    # print(df.columns)
    gdf = maps.make_as_well_gdf(df)
    return maps.find_disclosures_near_point(lat,lon,gdf,buffer_m=radius_m)
...
def show_disclosure_info(disclst,df):
    t = df[df.DisclosureId.isin(disclst)].copy()
    # save the full data set from these wells to allow users to download
    # t.to_csv('all_data_for_selected_wells.csv')
    t.api10 = t.apply(lambda x: make_disc_link(x),axis=1)
    dgb = t.groupby(['DisclosureId'],as_index=False)[['date','api10','APINumber','WellName',
                                                      'OperatorName','TotalBaseWaterVolume','no_chem_recs']].first()
    iShow(dgb[['date','OperatorName','api10','WellName','TotalBaseWaterVolume','no_chem_recs']])
    return t, dgb
...
def show_water_used(dgb):
    ...
    plt.savefig(os.path.join(out_dir,'water_use.jpg'))
...
def save_pdf_report(report_name,well_list,c_obj,lat, lon, radius):
    ...
    l = [] # list of flowables
    rgen = mpr.Report_gen(outfn = os.path.join(out_dir,'report_test.pdf',),
                          custom_title=report_name,
    ...
    items.append(Image(os.path.join(out_dir,'water_use.jpg'),width=400,height=300))
