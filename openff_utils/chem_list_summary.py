...
from openff_utils import handles as hndl


@lru_cache(maxsize=1)
def _load_cas_reference_data():
    """Load chemical reference tables once per process and cache them."""
    casdf = pd.read_parquet(hndl.repo_pickles_url + 'bgCAS.parquet')
    casingdf = pd.read_parquet(hndl.repo_pickles_url + 'cas_ing.parquet')
...
class ChemListSummary():
    ...
    def assemble_cas_df(self,use_remote=True):
        casdf, casingdf = _load_cas_reference_data()
